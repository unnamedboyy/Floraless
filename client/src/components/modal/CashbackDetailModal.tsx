    ...
    toast(
      (t) => (
        <div className="w-[320px]">
          <p className="font-semibold text-sm">Approve Cashback?</p>
          <p className="text-sm text-gray-500 mt-1">
            Cashback akan disetujui dan tidak dapat dibatalkan.
          </p>
          <div className="flex justify-end gap-2 mt-4">
            ...
            <button
              onClick={async () => {
                toast.dismiss(t.id);
                try {
                  await onApprove(data._id, buktiTf);
                  toast.success("Cashback berhasil diapprove");
                } catch (err: any) {
                  toast.error(
                    err?.response?.data?.message || "Gagal approve cashback"
                  );
                }
              }}
              className="px-3 py-2 rounded-xl bg-green-500 text-white text-sm hover:bg-green-600"
            >
              Setujui
            </button>
          </div>
          ...
          <div className="flex justify-end gap-2 mt-4">
            ...
            <button
              onClick={async () => {
                toast.dismiss(t.id);
                try {
                  await onReject(data._id, alasan);
                  toast.success("Cashback berhasil direject");
                } catch (err: any) {
                  toast.error(
                    err?.response?.data?.message || "Gagal reject cashback"
                  );
                }
              }}
  ...
  const handleUpload = async (
    ...
    try {
      ...
      if (file.size > 20 * 1024 * 1024) {
        toast.error("Ukuran maksimal 20MB");
        return;
      }
      ...
      const url = await uploadImage(file, "cashback");
          ...
          <Section title="Approve Cashback">
            ...
            <div className="space-y-6">
              ...
              <button
                ...
                disabled={loadingApprove || uploading || !buktiTf}
                className="
                  h-14
                  w-full
                  rounded-2xl
                ...
                "
              >
                {loadingApprove ? "Processing..." : "Approve Cashback"}
              </button>
          ...
          <Section title="Reject Cashback">
            ...
            <div className="space-y-6">
              ...
              <button
                ...
                "
              >
                {loadingReject ? "Processing..." : "Reject Cashback"}
              </button>

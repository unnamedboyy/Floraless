"use client";

import {
  CalendarDays,
  MapPin,
  User2,
  ClipboardList,
  Clock3,
  X,
  FileText,
  Camera,
  Wallet,
  Ticket,
  BadgeCheck,
} from "lucide-react";

import BaseModal from "@/components/form/BaseModal";

/* =========================================================
   TYPES
========================================================= */
interface Props {
  open: boolean;
  onClose: () => void;
  data: any;
}

/* =========================================================
   HELPERS
========================================================= */
const formatRupiah = (num: number) =>
  "Rp " + (num || 0).toLocaleString("id-ID");

/* =========================================================
   COMPONENT
========================================================= */
export default function DetailJadwalModal({ open, onClose, data }: Props) {
  if (!open || !data) return null;

  /* =====================================================
     DESTRUCTURING DATA (Jadwal & Ticket Relation)
  ===================================================== */
  const ticket = data.ticketId;
  const pelanggan = ticket?.pelangganId;
  const layanan = ticket?.layananId;
  
  // Mengambil detail acara (prioritas data jadwal, fallback ke data tiket)
  const namaAcara =
    data.nama_acara ||
    data.title ||
    ticket?.detail?.nama_acara ||
    layanan?.nama ||
    "-";

  const jenisAcara = ticket?.detail?.jenis_acara || "-";

  const lokasiAcara =
    data.lokasi ||
    ticket?.detail?.lokasi ||
    "-";

  const formattedDate = data.tanggal_acara || ticket?.detail?.tanggal_acara
    ? new Date(data.tanggal_acara || ticket.detail.tanggal_acara).toLocaleDateString(
        "id-ID",
        {
          weekday: "long",
          year: "numeric",
          month: "long",
          day: "numeric",
        }
      )
    : "-";

  const jamAcara =
    data.jam_mulai && data.jam_selesai
      ? `${data.jam_mulai} - ${data.jam_selesai}`
      : ticket?.detail?.jam_mulai && ticket?.detail?.jam_selesai
      ? `${ticket.detail.jam_mulai} - ${ticket.detail.jam_selesai}`
      : "-";

  const catatanAcara =
    data.catatan ||
    ticket?.detail?.catatan ||
    "Belum ada catatan";

  return (
    <BaseModal open={open} onClose={onClose} maxWidth="max-w-5xl">
      {/* =====================================================
          HEADER
      ===================================================== */}
      <div className="px-8 py-7 border-b border-slate-200 flex items-start justify-between">
        <div>
          <h2 className="text-[38px] leading-none font-bold tracking-tight text-[#0F172A]">
            Detail <span className="text-[#C9AE63]">Jadwal & Tiket</span>
          </h2>
          <p className="text-slate-500 text-sm mt-3">
            Informasi lengkap jadwal acara, data pelanggan, beserta detail tiket
          </p>
        </div>

        <button
          onClick={onClose}
          className="w-12 h-12 rounded-2xl border border-slate-200 flex items-center justify-center text-slate-500 hover:bg-slate-100 transition"
        >
          <X size={20} />
        </button>
      </div>

      {/* =====================================================
          BODY
      ===================================================== */}
      <div className="px-8 py-8 space-y-7 overflow-y-auto max-h-[75vh]">
        
        {/* =====================================================
            1. DETAIL ACARA
        ===================================================== */}
        <Section title="Detail Acara">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <FieldCard
              icon={<ClipboardList size={18} />}
              label="Nama Acara"
              value={namaAcara}
            />
            <FieldCard
              icon={<Ticket size={18} />}
              label="Jenis Acara"
              value={jenisAcara}
            />
            <FieldCard
              icon={<MapPin size={18} />}
              label="Lokasi"
              value={lokasiAcara}
            />
            <FieldCard
              icon={<CalendarDays size={18} />}
              label="Tanggal Acara"
              value={formattedDate}
            />
            <FieldCard
              icon={<Clock3 size={18} />}
              label="Jam Acara"
              value={jamAcara}
            />
            <div className="md:col-span-2">
              <FieldCard
                icon={<FileText size={18} />}
                label="Catatan Acara"
                value={catatanAcara}
                multiline
              />
            </div>
          </div>
        </Section>

        {/* =====================================================
            2. INFORMASI PELANGGAN & LAYANAN
        ===================================================== */}
        <Section title="Informasi Pelanggan & Layanan">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <FieldCard
              icon={<User2 size={18} />}
              label="Nama Pelanggan"
              value={pelanggan?.nama || "-"}
            />
            <FieldCard
              icon={<Camera size={18} />}
              label="Layanan Tiket"
              value={layanan?.nama || "-"}
            />
            <FieldCard
              icon={<Wallet size={18} />}
              label="Harga Layanan"
              value={formatRupiah(layanan?.harga || 0)}
            />
            <FieldCard
              icon={<BadgeCheck size={18} />}
              label="Status Tiket"
              value={ticket?.status || "Pending"}
            />
          </div>
        </Section>

        {/* =====================================================
            3. PENUGASAN INTERNAL (PIC / PEGAWAI)
        ===================================================== */}
        <Section title="Penugasan Tim">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <FieldCard
              icon={<User2 size={18} />}
              label="Pegawai / PIC Lapangan"
              value={data.pegawaiId?.nama || "Belum ditugaskan"}
            />
          </div>
        </Section>

        {/* =====================================================
            TIMESTAMP JADWAL
        ===================================================== */}
        <div className="rounded-[30px] border border-blue-200 bg-blue-50 p-5 flex items-start gap-4">
          <div className="w-11 h-11 rounded-2xl bg-blue-100 flex items-center justify-center text-blue-700 shrink-0">
            <Clock3 size={20} />
          </div>
          <div>
            <h4 className="text-sm font-semibold text-blue-900">Informasi Sistem</h4>
            <p className="text-sm text-blue-700 mt-1 leading-relaxed">
              Jadwal ini dibuat pada{" "}
              {data.createdAt ? new Date(data.createdAt).toLocaleString("id-ID") : "-"}
            </p>
          </div>
        </div>

      </div>
    </BaseModal>
  );
}

/* =========================================================
   SUB-COMPONENTS (SECTION & FIELD CARD)
========================================================= */
function Section({ title, children }: any) {
  return (
    <div className="border border-slate-200 rounded-[30px] p-6 bg-white space-y-5 shadow-sm">
      <h3 className="text-xl font-bold text-[#0F172A]">{title}</h3>
      {children}
    </div>
  );
}

function FieldCard({ label, value, icon, multiline = false }: any) {
  return (
    <div className="border border-slate-200 rounded-2xl p-5 bg-slate-50/70 space-y-3">
      <div className="flex items-center gap-2 text-slate-500">
        {icon}
        <span className="text-sm font-medium">{label}</span>
      </div>
      <div className={`text-[15px] font-semibold text-slate-900 ${multiline ? "leading-relaxed whitespace-pre-wrap" : ""}`}>
        {value}
      </div>
    </div>
  );
}
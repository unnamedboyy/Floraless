...
export const createTicket = async (req, res, next) => {
  try {
    ...
    if (!jam_mulai) throw { status: 400, message: "Jam mulai acara wajib diisi" };
    if (!jam_selesai) throw { status: 400, message: "Jam selesai acara wajib diisi" };
    ...
    /* ================= CREATE JADWAL ================= */
    try {
      await Jadwal.create({ ticketId: ticket._id, tanggal_acara: tanggal, status: "booked" });
      getIO().emit("jadwal:update");
    } catch (err) {
      if (err.code === 11000) throw { status: 400, message: "Tanggal tersebut baru saja dibooking pelanggan lain" };
      throw err;
    }
    ...
    Tanggal Acara : ${new Date(tanggal).toLocaleDateString("id-ID")}
    Jam Acara : ${jam_mulai} - ${jam_selesai}
    ...
    return next({ status: err.status || 500, message: err.message || "Gagal membuat ticket" });
  }
};
...
export const createTicketByAdmin = async (req, res, next) => {
  try {
    ...
    if (!pegawaiId) throw { status: 400, message: "Pegawai PIC wajib dipilih" };
    if (!jam_mulai) throw { status: 400, message: "Jam mulai acara wajib diisi" };
    if (!jam_selesai) throw { status: 400, message: "Jam selesai acara wajib diisi" };
    if (jam_mulai >= jam_selesai) throw { status: 400, message: "Jam selesai harus lebih besar dari jam mulai" };
    ...
    const bentrok = await Jadwal.findOne({ tanggal_acara: { $gte: startDate, $lte: endDate } });
    if (bentrok) throw { status: 400, message: "Tanggal tersebut sudah dibooking" };

    /* ================= CREATE TICKET ================= */
    const ticket = await Ticket.create({ pelangganId, pegawaiId, layananId, status: "approved" });
    ...
    /* ================= CREATE JADWAL ================= */
    await Jadwal.create({ ticketId: ticket._id, pegawaiId, tanggal_acara: tanggal, status: "booked" });
    getIO().emit("jadwal:update");

    /* ================= LOG ================= */
    await LogActivity.create({
      ticketId: ticket._id,
      action: "CREATE_TICKET_ADMIN",
      status: "approved",
      description: "Ticket dibuat oleh admin",
    });
...
export const getTickets = async (req, res, next) => {
  try {
    ...
    if (req.user.role === "pegawai") {
      const pegawai = await Pegawai.findOne({ userId: req.user.id });
    ...
    /* ================= FILTER TANGGAL ================= */
    if (tanggal) {
      const jadwals = await Jadwal.find({ tanggal_acara: new Date(tanggal) });
      const ticketIds = jadwals.map((j) => j.ticketId.toString());
      tickets = tickets.filter((t) => ticketIds.includes(t._id.toString()));
    }

    /* ================= DETAIL TICKET ================= */
    const ticketIds = tickets.map((ticket) => ticket._id);
    const details = await DetailTicket.find({ ticketId: { $in: ticketIds } });
    const detailMap = new Map(details.map((detail) => [detail.ticketId.toString(), detail]));

    const ticketsWithDetail = tickets.map((ticket) => ({
      ...ticket.toObject(),
      detail: detailMap.get(ticket._id.toString()) || null,
    }));
...
export const getTicketById = async (req, res, next) => {
  try {
    ...
    if (req.user.role === "pelanggan") {
      const pelanggan = await Pelanggan.findOne({ userId: req.user.id });
      if (!pelanggan || ticket.pelangganId._id.toString() !== pelanggan._id.toString()) {
        throw { status: 403, message: "Tidak memiliki akses" };
      }
    }
    ...
    res.json({ ticket, detail, jadwal, logs, summary });
  } catch (err) {
    next(err);
  }
};
...
export const approveTicket = async (req, res, next) => {
  try {
    ...
    if (!pegawaiId) return res.status(400).json({ message: "pegawaiId wajib diisi" });

    const updated = await Ticket.findByIdAndUpdate(id, { pegawaiId, status: "approved" }, { new: true })
      .populate("pegawaiId", "nama")
      .populate("pelangganId", "nama email")
      .populate("layananId", "nama harga");

    if (!updated) return res.status(404).json({ message: "Ticket tidak ditemukan" });
...
export const updateStatusTicket = async (req, res, next) => {
  try {
    ...
    if (!allowedStatus.includes(status)) throw { status: 400, message: "Status tidak valid" };
    ...
    if (status === "rejected") {
      await Jadwal.deleteOne({ ticketId: ticket._id });
      getIO().emit("jadwal:update");
    }
    ...
    await LogActivity.create({
      ...
      status,
      description: status === "rejected" ? `Ticket ditolak. Alasan: ${note || "-"}` : `Status diubah ke ${status}`,
    });
...
const getPaymentSummary = async (ticket) => {
  ...
  const payments = await Payment.find({ ticketId: ticket._id, status: "approved" });
  ...
  if (totalDibayar === 0) statusPembayaran = "unpaid";
  else if (totalDibayar < totalTagihan) statusPembayaran = "partial";
  else if (totalDibayar === totalTagihan) statusPembayaran = "paid";

  return { totalTagihan, totalDibayar, sisaTagihan, statusPembayaran, jumlahPembayaran: payments.length };
};
...
export const getTicketFullById = async (req, res, next) => {
  try {
    ...
    if (ticket.layananId) totalHarga = ticket.layananId.harga;

    payments.forEach((p) => {
      if (p.status === "approved") totalDibayar += p.jumlah;
    });

    const sisa = totalHarga - totalDibayar;

    let paymentStatus = "unpaid";
    if (totalDibayar > 0 && totalDibayar < totalHarga) paymentStatus = "partial";
    if (totalDibayar >= totalHarga) paymentStatus = "paid";

    const paymentSummary = { totalHarga, totalDibayar, sisa, status: paymentStatus };

    const review = await Review.findOne({ ticketId: id });
    const claims = await CashbackClaim.find({ pelangganId: ticket.pelangganId._id }).sort({ createdAt: -1 });
    const logs = await LogActivity.find({ ticketId: id }).sort({ createdAt: -1 });
    const portfolio = await Portfolio.findOne({ ticketId: ticket._id, isActive: true });
...
export const addTicketLog = async (req, res, next) => {
  try {
    const { id } = req.params;
